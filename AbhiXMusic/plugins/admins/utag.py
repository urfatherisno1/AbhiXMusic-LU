...
async def tag_all_users(_, message):
    ...
    while f:
        if SPAM_CHATS.get(chat_id) == False:
            await message.reply_text("**ᴜɴʟɪᴍɪᴛᴇᴅ ᴛᴀɢɢɪɴɢ sᴜᴄᴄᴇssғᴜʟʟʏ sᴛᴏᴘᴘᴇᴅ.Nᴏᴡ ||Jᴏɪɴ|| Tᴏ ||@imagine_iq||**")
            break
        usernum = 0
        usertxt = ""
        try:
...
async def stop_tagging(_, message):
    ...
    if SPAM_CHATS.get(chat_id) == True:
        ...
        await message.reply_text("**Uɴʟɪᴍɪᴛᴇᴅ Tᴀɢɢɪɴɢ Sᴜᴄᴄᴇꜱꜱғᴜʟʟʏ Sᴛᴏᴘᴘᴇᴅ Nᴏᴡ ||Jᴏɪɴ|| Tᴏ ||@imagine_iq||**")
